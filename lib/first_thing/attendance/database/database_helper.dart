...
class DatabaseHelper {
  ...
  Future<int> insert(LaboursAttendance labour) async {
    final db = await _dbFuture;
    return db.insert(_tableItems, labour.toMap());
  }

  Stream<List<LaboursAttendance>> getAllItems() async* {
    final db = await _dbFuture;
    yield* db
        .createQuery(_tableItems)
        .mapToList((map) => LaboursAttendance.fromMap(map));
  }

  Future<bool> update(LaboursAttendance item) async {
    final db = await _dbFuture;
  ...
  Future<bool> remove(LaboursAttendance item) async {
    final db = await _dbFuture;
